    ...
    <VCard class="mb-6">
      <VCardText class="d-flex align-center justify-space-between">
        <div class="d-flex align-center">
          ...
          <div>
            <VCardTitle class="pa-0">My Technician Profile</VCardTitle>
            <VCardSubtitle class="pa-0">
              View and manage your technician information
            </VCardSubtitle>
          </div>
        </div>
    ...
    <div v-else>
      <VRow>
        ...
        <VCol cols="12" md="4">
          <!-- Profile Card -->
          <VCard class="mb-6">
            <VCardText class="text-center pa-">
              <VAvatar
                size="120"
                color="primary"
                variant="tonal"
              ...
              <div class="text-left">
                ...
                <div class="d-flex align-center mb-3">
                  ...
                  <span>{{ technician?.phone || user?.phone || 'Not provided' }}</span>
                </div>
          ...
          <VCard class="mb-6">
            <VCardTitle class="d-flex align-center justify-space-between">
              <div class="d-flex align-center">
                ...
                My Assigned Sites & Installations
              </div>
              <VBtn
                color="primary"
                size="small"
            ...
            <VCardText>
                  ...
                  <tr v-for="site in assignedSites" :key="site.id">
                    ...
                    <td>
                      <div class="d-flex gap-1">
                        ...
                        <VTooltip v-if="userPermissions.includes('assign sites')">
                          <template #activator="{ props }">
                            <VBtn
                              v-bind="props"
                              icon
              ...
              <div v-else class="text-center py-8">
                ...
                <p class="text-disabled mb-4">You haven't been assigned to any sites yet</p>
                <VBtn
                  color="primary"
      ...
      <!-- Assign Site Dialog (Only show if user has permission) -->
      <VDialog v-model="isAssignSiteDialogVisible" max-width="600" v-if="userPermissions.includes('assign sites')">
        <VCard>
          <VCardTitle class="d-flex align-center">
...
// Get technicianId from session user data (assuming you have this in your store or local storage)
const technicianId = computed(() => {
  // Option 1: If you have a store with user data
  // return useUserStore().userData?.id
  
  // Option 2: If you have user data in localStorage
...
const fetchTechnicianDetails = async () => {
  if (!technicianId.value) {
    error.value = 'No technician ID found in user session'
    loading.value = false
    return
  }
  ...
  error.value = null
  try {
    const response = await api.get(`/technician-user-data/${technicianId.value}`)
    ...
    if (data.success) {
      ...
      if (!data.user && !data.technician) {
        console.warn('No technician data found for ID:', technicianId.value)
      }
    } else {
