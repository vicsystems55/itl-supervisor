        ...
        <VCol cols="12" md="4">
          ...
          <VCard class="mb-6">
            <VCardText class="text-center pa-6">
              <VAvatar size="120" color="primary" variant="tonal" class="mb-4">
              ...
              <div class="text-left">
                ...
                <div class="d-flex align-center mb-3">
                  ...
                  />
                  <span>{{
                    technician?.id_card_number || "Not provided"
                  }}</span>
                </div>
              </div>
            </VCardText>
            ...
            <VCardText>
              <VTable v-if="assignedSites.length">
                ...
                <tbody>
                  <tr v-for="site in assignedSites" :key="site.id">
                    ...
                    <td>{{ site.location }}</td>
                    <td>
                      <VChip :color="getStatusColor(site.status)" size="small">
                        {{ site.status }}
                      </VChip>
                    </td>
              ...
              <div v-else class="text-center py-8">
                ...
                <p class="text-disabled mb-4">
                  This technician hasn't been assigned to any sites yet
                </p>
                <VBtn
                  color="primary"
                  prepend-icon="tabler-plus"
...
const summaryStats = computed(() => [
  ...
  {
    title: "Completed",
    value: assignedSites.value.filter((site) => site.status === "Completed")
      .length,
    icon: "tabler-check",
    color: "success",
    trend: "+1",
  ...
  {
    title: "In Progress",
    value: assignedSites.value.filter((site) => site.status === "In Progress")
      .length,
    icon: "tabler-clock",
    color: "warning",
    trend: "0",
  ...
  {
    title: "Pending",
    value: assignedSites.value.filter((site) => site.status === "Pending")
      .length,
...
const unassignSite = async (siteId) => {
  if (confirm("Are you sure you want to unassign this site?")) {
    try {
      ...
      assignedSites.value = assignedSites.value.filter(
        (site) => site.id !== siteId
      );
...
const initializeMap = () => {
  ...
  assignedSites.value.forEach((site) => {
    if (site.coordinates) {
      L.marker(site.coordinates).addTo(map).bindPopup(`
